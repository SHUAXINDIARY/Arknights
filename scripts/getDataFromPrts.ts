...
import { dataMap as previousDataMap } from '../src/data/AutoGenerateData'
...
const prependLatestData = <T>(latestData: T[], existingData: T[]) => {
  const seen = new Set<string>();

  return [...latestData, ...existingData].filter((item) => {
    const key =
...
(async () => {
  const dataMap = {} as Record<string, ExtractedData>;
  ...
  const mergedDataMap = Object.fromEntries(
    Object.entries(dataMap).map(([key, latestData]) => [
      key,
      prependLatestData(latestData, previousDataMap[key as keyof typeof previousDataMap] || []),
    ])
  );
  await fs.writeFileSync('./src/data/AutoGenerateData.ts', `export const dataMap = ${JSON.stringify(mergedDataMap)}`)
})();
