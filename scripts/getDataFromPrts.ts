...
type PageExtractor = () => ExtractedData | Promise<ExtractedData>;
type SkinRecord = { skinName: string; [key: string]: unknown };
...
type PrtsTask = {
  ...
  name: string;
  fun?: PageExtractor;
  customFun?: (browser: Browser, params: PrtsTask) => Promise<ExtractedData>;
};

const task: readonly PrtsTask[] = [
  {
  ...
  },
  {
    ...
    customFun: async (browser: Browser, params: PrtsTask) => {
...
(async () => {
  ...
  const skin: SkinRecord[] = saveSkin(dataMap._skinList);
  dataMap.memberNameAvatarMap = operator;
  dataMap._skinList = skin.map((item) => ({ ...item, skinName: item.skinName.replaceAll(' ', '_') }));
